...
- Reusable concepts now live in `concepts.hpp` and are re-exported by the lightweight `core.hpp` umbrella; the generic `dependent_false` helper is split into `dependent_false.hpp` so repository type declarations do not pull range concepts by side effect. Reusable cv-ref-normalized and const-normalized template constraints use `SameAsIgnoringCvref` and `UnsignedIntegralSameAsIgnoringConst`; repository context constraints also expose `CanonicalizableContextFor` so concept annotations stay in the `template<...>` head where possible. Raw pools, pool-backed sets, view infrastructure, closed interval helpers, generic hash/equality helpers, comparators, formatters, and associative container aliases include lighter direct dependencies instead of the heavier declaration aliases where possible.
- A lightweight opt-in umbrella now lives in `core.hpp`. It no longer includes filesystem/file-I/O helpers; `path.hpp` stays explicit and the broad `common.hpp` compatibility umbrella includes it directly.
- Associative container aliases now live in `associative_containers.hpp` and are re-exported by `containers.hpp`; `declarations.hpp` is kept as a declaration-only shim. The legacy `unordered_set.hpp` helper shim no longer includes the unused standard unordered-set header.
- Heavier pool and storage helpers are grouped by `containers.hpp`.
- Reusable third-party adapters are grouped by `adapters.hpp`. The umbrella now re-exports comparator and formatter split headers, with bitset-specific comparator headers including the bitset API they specialize and avoiding a duplicate direct dynamic-bitset edge. The TBB execution context validates requested thread counts before initializing its task arena.
- Tyr build-tree adapters are grouped by `project_adapters.hpp`, which includes `project_path.hpp` and `json_suite.hpp`.
- Repository/view infrastructure is grouped by `repository_types.hpp`. The cista view adapter headers now rely on `types_utils.hpp` for repository type declarations instead of redundantly including both `types.hpp` and `types_utils.hpp`; `repository_types.hpp` no longer repeats the `types_utils.hpp` include because the grouped view adapters already provide it.
- Generic path/file helpers live in `path.hpp`.
- Checked conversion to the common `uint_t` index type lives in `config.hpp` and is used by common index-producing containers before narrowing sizes. Direct users now increasingly include `associative_containers.hpp`, `concepts.hpp`, `config.hpp`, `types.hpp`, `hash.hpp`, and `equal_to.hpp` directly instead of the declaration shim when they only need aliases, concepts, primitive config, repository type declarations, or concrete gtl containers. `declarations.hpp` is now declaration-only and no longer pulls associative container aliases, concepts, or primitive config by side effect; the broad `common.hpp` umbrella no longer includes that redundant declaration shim.
...
- Range hashing/equality/ordering helpers are centralized in `hash.hpp`, `equal_to.hpp`, and `comparators.hpp`. Cista-specific hash, equality, and ordering specializations now live in role-specific `cista_hash.hpp`, `cista_equal_to.hpp`, and `cista_ordering.hpp`, keeping the generic helper headers free of cista container includes for non-cista users. The generic helper headers also shed stale standard includes after that split, and canonicalization keeps direct generic comparator/equality includes instead of depending on the full cista comparator umbrella.
...
- Observer-pointer hash, equality, and ordering specializations now live in role-specific `observer_ptr_hash.hpp`, `observer_ptr_equal_to.hpp`, and `observer_ptr_ordering.hpp`; all three use pointee semantics, keeping generic `hash.hpp`, `equal_to.hpp`, and `comparators.hpp` from pulling `observer_ptr.hpp` for non-observer-pointer users.
- Bitset-specific hash, equality, formatting, and ordering specializations now live in role-specific dynamic-bitset adapter headers. This keeps generic `hash.hpp`, `equal_to.hpp`, `formatter.hpp`, and `comparators.hpp` from pulling the full dynamic bitset/Boost dependency into every generic user while preserving opt-in coverage through `adapters.hpp`.
- Cista-specific fmt support now lives in `cista_formatters.hpp`, and bitset-specific fmt support lives in `dynamic_bitset_formatters.hpp`. This keeps generic `formatter.hpp` from pulling cista containers or dynamic bitset/Boost support into every formatter user; `adapters.hpp` and domain formatters include the split headers explicitly where needed.
- Associative container fmt support now lives in `associative_container_formatters.hpp`. The split header forward-declares `Hash` and `EqualTo` instead of including their implementations, and keeps generic `formatter.hpp` from pulling `Hash`, `EqualTo`, and `gtl/phmap.hpp` into every formatter user while preserving broad adapter coverage through `adapters.hpp`. The generic formatter header also now sheds stale standard and fmt includes left behind by the adapter split, and no longer pulls `types.hpp`, `index_mixins.hpp`, or `vector.hpp` just to declare dependent repository index and view formatters.
- Common type-family ordering boundaries now have compact `*_ordering.hpp` headers for cista containers/views, dynamic bitsets, observer pointers, block-array views, raw vector views, and segmented vectors. The existing `*_comparators.hpp` headers remain compatibility shims for current users, while `adapters.hpp` consumes the ordering boundary names.
- `index_mixins.hpp` now compares its stored `uint_t` value directly instead of including generic `equal_to.hpp`, keeping repository index declarations from pulling equality adapter dependencies by side effect.
- `delta_kpkc_graph.hpp` now includes `associative_containers.hpp` and the bitset adapter directly instead of relying on hash/equality side effects for `UnorderedMap<BitsetSpan<...>>`.
