...
- Observer-pointer hash/equality specializations now live in `observer_ptr_comparators.hpp`, keeping generic `hash.hpp` and `equal_to.hpp` from pulling `observer_ptr.hpp` for non-observer-pointer users.
- Bitset-specific hash/equality specializations now live in `dynamic_bitset_comparators.hpp`. This keeps generic `hash.hpp` and `equal_to.hpp` from pulling the full dynamic bitset/Boost dependency into every hash/equality user while preserving opt-in coverage through `adapters.hpp`.
...
- Block-array-specific ordering specializations now live in `block_array_comparators.hpp`. This keeps generic `comparators.hpp` from pulling block-array storage into every `Less` user, and the block-array comparator header forward-declares repository views instead of pulling `types.hpp`; `common.hpp` still preserves the broad compatibility umbrella.
