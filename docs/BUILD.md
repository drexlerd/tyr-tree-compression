...
- `pyyggdrasil>=0.0.21,<0.1` provides shared libraries, headers, and CMake packages for common native dependencies.
- `pypddl>=1.0.23,<1.1` provides Loki's PDDL parser library, headers, and CMake package.
- `pypddl-datasets>=0.0.5,<0.1` provides the PDDL benchmark data used by the C++ test and profiling fixtures (resolved from its cache at CMake configure time).
...
```console
uv venv
uv pip install 'pyyggdrasil>=0.0.21,<0.1' 'pypddl>=1.0.23,<1.1' 'pypddl-datasets>=0.0.5,<0.1'
```
...
.venv/bin/python -m pip install 'pyyggdrasil>=0.0.21,<0.1' 'pypddl>=1.0.23,<1.1' 'pypddl-datasets>=0.0.5,<0.1'
```
