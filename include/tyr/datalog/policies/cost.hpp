...
class RuleBindingCostOverridePolicy
{
public:
    using CostMap = ygg::UnorderedMap<::tyr::formalism::datalog::RuleBindingView, Cost>;

    RuleBindingCostOverridePolicy() = default;
    explicit RuleBindingCostOverridePolicy(CostMap costs) : m_costs(std::move(costs)), m_prefix_costs() {}
...
static_assert(CostPolicyConcept<LiftedRuleCostPolicy>);
static_assert(CostPolicyConcept<RuleBindingCostOverridePolicy>);
static_assert(MutableCostPolicyConcept<RuleBindingCostOverridePolicy>);
