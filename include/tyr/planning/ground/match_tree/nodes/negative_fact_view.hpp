...
#ifndef TYR_PLANNING_GROUND_MATCH_TREE_NODES_NEGATIVE_FACT_VIEW_HPP_
#define TYR_PLANNING_GROUND_MATCH_TREE_NODES_NEGATIVE_FACT_VIEW_HPP_
...
private:
    const C* m_context;
    ygg::Index<planning::match_tree::NegativeFactSelectorNode<Tag>> m_handle;

public:
    View(ygg::Index<planning::match_tree::NegativeFactSelectorNode<Tag>> handle, const C& context) noexcept : m_context(&context), m_handle(handle) {}

    const auto& get_data() const noexcept { return get_repository(*m_context)[m_handle]; }
    const auto& get_context() const noexcept { return *m_context; }
    const auto& get_handle() const noexcept { return m_handle; }

    auto get_fact() const noexcept { return ygg::make_view(get_data().fact, m_context->get_formalism_repository()); }
