...
void benchmark_gbfs_lazy_lmcut(benchmark::State& state, const BenchmarkCase& benchmark_case)
{
    ...
    for (auto _ : state)
    {
        ...
        auto heuristic = p::LMCutHeuristic<p::LiftedTag>::create(task, execution_context);
...
int main(int argc, char** argv)
{
    ...
    for (const auto& benchmark_case : load_cases())
    {
        benchmark::RegisterBenchmark((benchmark_case.name + "/gbfs_lazy").c_str(),
                                     [benchmark_case](benchmark::State& state) { benchmark_gbfs_lazy_lmcut(state, benchmark_case); });
    }
