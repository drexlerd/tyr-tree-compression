...
StateRepository<GroundTag>::StateRepository(ygg::uint_t index, TaskPtr<GroundTag> task, AxiomEvaluatorPtr<GroundTag> axiom_evaluator) :
    m_index(index),
...
StateView<GroundTag> StateRepository<GroundTag>::create_state(const std::vector<ygg::Data<fp::FDRFact<f::FluentTag>>>& fluent_facts,
...
ygg::SharedObjectPoolPtr<UnpackedState<GroundTag>> StateRepository<GroundTag>::get_unregistered_state()
{
...
StateView<GroundTag> StateRepository<GroundTag>::register_state(ygg::SharedObjectPoolPtr<UnpackedState<GroundTag>> state)
{
    if (m_axiom_evaluator)
        m_axiom_evaluator->compute_extended_state(*state);

    state->set(m_packed_states
                   .insert(ygg::Data<State<GroundTag>>(ygg::Index<State<GroundTag>>(m_packed_states.size()),
