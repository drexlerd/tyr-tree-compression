    ...
    for (ygg::uint_t i = 0; i < arity; ++i)
    {
        ...
        for (ygg::uint_t j = i + 1; j < arity; ++j)
        {
            result = intersect(result,
                               (*this)[EdgeAssignment(::tyr::formalism::ParameterIndex(i),
                                                      objects[i].get_index(),
                                                      ::tyr::formalism::ParameterIndex(j),
                                                      objects[j].get_index())]);
            if (empty(result))
                return result;
        }
...
ygg::ClosedInterval<ygg::float_t> FunctionAssignmentSet<T>::at(::tyr::formalism::datalog::FunctionBindingView<T> binding) const noexcept
{
    ...
    for (ygg::uint_t i = 0; i < arity; ++i)
    {
        ...
        {
            result = intersect(result,
                               at(EdgeAssignment(::tyr::formalism::ParameterIndex(i),
                                                 objects[i].get_index(),
                                                 ::tyr::formalism::ParameterIndex(j),
                                                 objects[j].get_index())));
            if (empty(result))
...
AssignmentSets::AssignmentSets(const TaggedAssignmentSets<::tyr::formalism::StaticTag>& static_sets, const TaggedAssignmentSets<::tyr::formalism::FluentTag>& fluent_sets) :
