...
void check_statistics(const SearchStatistics& expected, const SearchCase& test_case)
{
    ...
    auto event_handler = p::brfs::DefaultEventHandler<Kind>::create();
...
TEST_P(BrfsStatisticsTest, StatisticsMatchFixture)
{
    ...
    for (const auto& [key, expected] : test_case.configs)
    {
        SCOPED_TRACE(key);
        if (key == "ground")
            check_statistics<p::GroundTag>(expected, test_case);
        else
            check_statistics<p::LiftedTag>(expected, test_case);
    }
}

INSTANTIATE_TEST_SUITE_P(TyrPlanningBrfsStatistics,
                         BrfsStatisticsTest,
                         ::testing::ValuesIn(load_search_cases("tests/unit/planning/algorithms/statistics/brfs.json")),
