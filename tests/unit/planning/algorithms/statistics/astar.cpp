...
void check_statistics(const SearchStatistics& expected, const SearchCase& test_case, const std::string& heuristic_name)
{
    ...
    auto event_handler = p::astar_eager::DefaultEventHandler<Kind>::create();
...
INSTANTIATE_TEST_SUITE_P(TyrPlanningAstarStatistics,
                         AstarStatisticsTest,
                         ::testing::ValuesIn(load_search_cases("tests/unit/planning/algorithms/statistics/astar.json")),
