...
/// One fixture case: expected statistics per configuration key. A configuration key is
/// "<kind>" (heuristic-free searches) or "<kind>_<heuristic>", e.g. "ground" or "lifted_hadd";
/// any case key whose object value holds a "num_generated" member is parsed as a configuration.
struct SearchCase
{
    std::string name;
...
/// Split a configuration key into (kind, heuristic); the heuristic is empty for heuristic-free keys.
...
template<planning::TaskKind Kind>
planning::HeuristicPtr<Kind> create_search_heuristic(const std::string& name, const SearchContext<Kind>& context)
{
    if (name == "blind")
        return planning::BlindHeuristic<Kind>::create();
    if (name == "hmax")
        return planning::MaxRPGHeuristic<Kind>::create(context.task, context.execution_context, planning::CostMode::GENERAL);
    if (name == "hadd")
        return planning::AddRPGHeuristic<Kind>::create(context.task, context.execution_context, planning::CostMode::GENERAL);
    if (name == "hff")
        return planning::FFRPGHeuristic<Kind>::create(context.task, context.execution_context, planning::CostMode::GENERAL);
    if (name == "hlmcut")
        return planning::LMCutHeuristic<Kind>::create(context.task, context.execution_context, planning::CostMode::GENERAL);
